...
Since most of the architecture, design and planning questions have already been answered during the development of the prototype, we do not expect any serious technical or architectural issues to surface at this point. Additionally, the company support from Broadcom Inc.~prevents financing and motivation issues. We therefore assume that it is very probable that the project will be completed as planned. 

The work packages have been assigned to individual team members based on long-term planning. The assignments and corresponding deadlines are listed in \cref{table}, and summarized by a diagram in~\cref{fig:gantt-pokus}.
...
\Mdef{1}{Research and analysis}{}
	...
	{Analysis of HLASM specification, available code and discussion with HLASM users.}
...
\Mdef{8}{Feature testing}{After the M8 milestone, the software should be well tested, stable and able to run seamlessly on any major platform.}
	\WPdefe{24}{Testing}
	\WPdef{25}{Multi-platform deployment}{Deployment on Windows, Linux and MacOS}
...
\newcommand{\M}[2]{\multirow{#1}{0.8cm}{\centering M#2}}
...
\begin{landscape}
	\begin{figure}
		...
		\coordinate (today) at ([xshift=-5mm] t4);
		
		\draw[draw=black!25, dashed] (today|- wp1) to (today |- wp27); %sync with previous
		\node[month] at (today|-wp30) {today};
...
\itemwp Benchmark \people{Marcel}
\eenum
...
\begin{landscape}
	\begin{figure}
		...
		\begin{ganttchart}[vgrid={draw=none, dotted}, x unit = 1.2cm]{1}{12}
			...
			\ganttvrule[vrule offset=0.5]{today}{1}
