...
Once the project is built, there are two test executables in the \TT{bin/} subdirectory of the build folder: \TT{library\_test} and \TT{server\_test}. Just run both of them to verify the build. The tests must be run from the bin folder, because they need to load resources from \TT{bin/test/} directory.
