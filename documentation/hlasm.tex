...
Using ORG instruction, location counter value can be arbitrarily changed. It can be used to move counter value backward or forward (with restriction of counter underflow) to set the next address. This means that user can generate some code, move counter backwards and overwrite it with a \emph{deeper} meaning. Then, ORG instruction has option to set location counter to the next available untouched address to continue in object creation.
...
File that generates object code is called \emph{open-code} file. It is the entry file of the HLASM compiler. Moreover, each open-code can have in-file dependencies. Specifically, they are:
\begin{itemize}
	\item \textbf{External Macro definitions}
	\item \textbf{Copy members}
\end{itemize}
They are not treated as an open-code files because they do not directly generate object code. Rather, they serve as statement sequences that are included into open-code to specific places to have specific meaning.
