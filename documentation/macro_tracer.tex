...
The attribute variable represents attributes of all types of symbols. It does not access context, and it is only used by the rest of variables to show their attributes.

\section{Debugger}

The debugger component is the core of the macro tracer implementation. When the user starts debugging, the method \TT{launch} is called from the language server component. The debugger creates analyzer and starts the analysis in a separate thread. The debugger implements processor tracer interface, which only has one method --- \TT{statement}. The analyzer calls the \TT{statement} method every time a next statement is about to be processed.
