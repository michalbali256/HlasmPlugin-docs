...
\begin{itemize}
	...
	\item \emph{Instruction Format Validators} (see \cref{lab06:instr_valid})
\end{itemize}
...
As format of some instruction kinds can be rather complicated, instruction processors contain \emph{Instruction format validators} (see \cref{lab06:instr_valid}). They check the statement to validate correctness of used operand format.

During the work of the processors, they encounter \emph{expression} structures (see \cref{lab06:expr}). Expressions need to be evaluated for processing to be correctly finished.
