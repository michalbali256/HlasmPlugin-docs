...
\subsectionSrc{Types of statement processors}{parser\_library/src/processing/statement\_processors/statement\_processor.h}
...
During processing, statement processing kinds can be nested. Hence, statement processors are dynamically assigned to the manager when needed and removed from it when they finish.

\subsubsectionSrc{Statement}{parser\_library/src/context/hlasm\_statement.h,parser\_library/src/processing/statement.h,parser\_library/src/semantics/statement.h}
...
\subsectionSrc{Types of statement providers}
{parser\_library/src/processing/statement\_providers/statement\_provider.h}
\label{lab06:sect_prov}

Due to the macro definitions, copy file includes and statement generation, it is difficult to state which statement should be processed next. For this reason, we define abstraction for various sources of statements called \emph{statement providers}.
...
\subsubsection{Statement pasing}
...
At first, it is used during statement passing. In some cases provider is requested a concrete format of a string-stored statement. The string is re-parsed with the according format. Then the field is returned back to the statement provider. 
