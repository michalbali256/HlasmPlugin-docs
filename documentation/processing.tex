...
Processing manager is a major component in the processing of a HLASM source file. It decides which stream of statements is about to be processed and assigns it to the correct processor. It contains components responsible for instruction interpretation as well as instruction format validation (see \cref{fig06:proc_mngr}). 
...
Let us enumerate the main components that are used in the processing manager:
\begin{itemize}
	\item \emph{Statement Provider} (see \cref{lab06:sect_prov})
	\item \emph{Statement Processor} (see \cref{lab06:sect_proc})
	\item \emph{Instruction Processor} (see \cref{lab06:instr_proc})
	\item \emph{Instruction Format Validators} (see \cref{checker})
\end{itemize}
...
In contrary to statement processors, statement providers are stored in the array based on their priority (lower index, greater priority):
...
Based on the priority, each cycle of the processor's main loop, providers are asked for statements to provide. That is because after each cycle, a provider with greater priority than the current one can be activated. 
...
As format of some instruction kinds can be rather complicated, instruction processors contain \emph{Instruction format validators} (see \cref{checker}). They check the statement to validate correctness of used operand format.

During the work of the processors, they encounter \emph{expression} structures (see \cref{ca_expr:logic}, \cref{mach_expr}). Expressions need to be evaluated for processing to be correctly finished.
