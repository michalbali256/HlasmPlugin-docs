...
\begin{enumerate}
	\item After opening a HLASM file for the first time, two popups display. Select \TT{Create pgm\_conf.json with current program} and \TT{Create empty proc\_grps.json}. The two configuration files are then created with default values. They are written into the \TT{.hlasmplugin} subfolder.
